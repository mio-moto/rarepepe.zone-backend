...
import { APIApplicationCommandInteraction, APIBaseInteraction, APIWebhook, ApplicationCommandType, GatewayReceivePayload, InteractionType, WebhookType } from "discord-api-types/v10";
...
import { EightPepeCommand } from "./constants";
...
export default async <const Prefix extends string | undefined>({ prefix = '/webhooks/discord' }: { prefix: Prefix }, environment: Environment) => {
    const config = await loadDiscordConfig();
    const { allowedApplicationIds, publicKey, applicationId, token, administrators } = config;
    const discordClient = buildDiscordAPIClient(applicationId, token);
    const discordEnvironment = { client: discordClient, ...environment, discord: { client: discordClient, config: config } };
    const handleWebhookMessage = await buildDiscordEventBus(discordEnvironment);

    return new Elysia({ prefix })
        .post('/', async ({ body, set, request, headers }) => {
