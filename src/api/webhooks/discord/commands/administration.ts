...
import { interactionName } from "../interactionDiscriminators";


const collectPluginDescriptors = (plugins: Plugin[]) => {
    const globalCommands: (RESTPostAPIChatInputApplicationCommandsJSONBody | RESTPostAPIContextMenuApplicationCommandsJSONBody)[] = []
    const localCommands: Record<string, (RESTPostAPIChatInputApplicationCommandsJSONBody | RESTPostAPIContextMenuApplicationCommandsJSONBody)[]> = { }
    ...
    return [globalCommands as ApplicationCommandUpdateRequest[], localCommands as Record<string, ApplicationCommandUpdateRequest[]>] as const;
}
...
export const buildCycleCommands = async (getRabs: () => Rabscuttle, {discord}: DiscordEnvironment) => {
    ...
    const cycleCommands: InteractionPlugin = {
        ...
        onNewInteraction: async ({reply, interaction}, {discord}) => {
            ...
            if(!member || !discord.config.administrators.some(x => x.tag === member.discriminator && x.username === member.username)) {
            ...
            const report = `Global: [${registeredGlobalCommands.join(", ")}]\n`
            ...
            reply({ content: `Overwritten and registered:\n\`\`\`json\n${JSON.stringify(reportObject, null, 1)}\n\`\`\``});
        }
    }
