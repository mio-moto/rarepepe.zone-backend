import { APIActionRowComponent, APIApplicationCommandInteractionDataStringOption, APIBaseComponent, APIChatInputApplicationCommandInteraction, APIInteractionResponse, APIInteractionResponseCallbackData, APIInteractionResponseChannelMessageWithSource, APIMessageActionRowComponent, ApplicationCommandOptionType, ButtonStyle, ComponentType } from "discord-api-types/v10";
...
const alwaysExcepts = () => { throw new Error("not implemented"); }
...
const embedUltra = (ultraIcon: string, pepe: UltraResult, interaction: APIChatInputApplicationCommandInteraction): APIInteractionResponseCallbackData => {
    return {
        embeds: [{
            title: `${pepe.filename}`,
            color: 0xDDDDDD,
...
const embedRare = (rareIcon: string, pepe: RareResult, interaction: APIChatInputApplicationCommandInteraction): APIInteractionResponseCallbackData => {
    return {
        embeds: [{
            author: {
                name: "A RARE PEPE",
...
export const buildEightPepe = async (environment: DiscordEnvironment) => {
    ...
    const EightPepe: InteractionPlugin & ComponentPlugin = {
        ...
        onNewInteraction: async (interaction) => {
            ...
            const phrase = interaction.interaction.data.options?.find(x => x.name === "phrase") as APIApplicationCommandInteractionDataStringOption | null;
            ...
            switch (result.type) {
                case "ultra":
                    ...
                    break;
                case "rare":
                    message = embedRare(rareIcon, result, interaction.interaction);
                    break;
                case "simple":
            ...
            if (result.type === "rare" || result.type === "simple") {
                const sentMessage = await environment.discord.client.interactions.response.getOriginal(interaction.interaction.token);
                await voter.beginVoting(sentMessage.data.channel_id, sentMessage.data.id);
            }
        },
        onNewButtonClick: async (interaction, environment) => {
