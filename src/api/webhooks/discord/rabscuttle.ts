...
import { APIApplicationCommandAutocompleteInteraction, APIApplicationCommandInteraction, APIApplicationCommandInteractionData, APIChatInputApplicationCommandInteraction, APIContextMenuInteraction, APIInteractionResponseCallbackData, ApplicationCommandType, InteractionType, RESTPostAPIApplicationCommandsJSONBody, RESTPostAPIChatInputApplicationCommandsJSONBody, RESTPostAPIContextMenuApplicationCommandsJSONBody } from "discord-api-types/v10"
import { APIUnknownInteraction, AnyGatewayResult } from ".";
import { DiscordAPIClient } from "./apiClient";
import { ApplicationInteractionClient, AutocompleteInteractionClient, ComponentInteractionClient, ContextMenuInteractionClient, ModalInteractionClient, dispatchInteractionResponseClient } from "./pluginClient";
...
const register = (plugin: Plugin) => {
    ...
    if (isComponentPlugin(plugin)) {
        const idCollisions = plugin.publishedComponentIds.filter(id => plugins.components.some(x => x.publishedComponentIds.includes(id)));
    ...
    if (isModalPlugin(plugin)) {
        ...
        for (const idCollision of idCollisions) {
            ...
            logger.warn(`Modal ID '${idCollision}' of [${plugin.name}] collides with these plugins publishing the same custom id [${pluginCollision.join(", ")}]`);
        }
