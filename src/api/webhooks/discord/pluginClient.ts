...
import { APIInteractionResponse, InteractionType, APIBaseInteraction, APIInteractionResponseCallbackData, APIChatInputApplicationCommandInteraction, APIContextMenuInteraction, APIApplicationCommandAutocompleteInteraction, APIPingInteraction, MessageFlags, RESTPostAPIWebhookWithTokenJSONBody, APIModalSubmitInteraction, APIMessageComponentInteraction, APICommandAutocompleteInteractionResponseCallbackData, APIModalComponent, APIModalInteractionResponse, APIModalInteractionResponseCallbackData } from "discord-api-types/v10";
import { APIUnknownInteraction, AnyGatewayResult } from ".";
import { createInteractionResponse_DeferredChannelMessage, createInteractionResponse_DeferredMessageUpdate, createInteractionResponse_ChannelMessage, createInteractionResponse_Pong, createModalInteractionResponse, createApplicationCommandResponse_Autocomplete, createInteractionResponse_UpdateMessage } from "./messagePayloads";
import { interactionName as getInteractionName, interactionTypeName as getInteractionTypeName, isApplicationInteraction, isAutocompleteInteraction, isComponentInteraction, isContextMenuInteraction, isModalInteraction, isPingInteraction } from "./interactionDiscriminators";
...
type InteractionDefer = Parameters<typeof createInteractionResponse_DeferredChannelMessage>[0] | Parameters<typeof createInteractionResponse_DeferredMessageUpdate>;
type BaseInteraction<Type extends InteractionType, Data> = APIBaseInteraction<Type, Data>;


export interface PluginClient<Response extends InteractionResponse, Defer extends InteractionDefer, Interaction extends BaseInteraction<Type, Data>, Type extends InteractionType, Data> {
        ...
        modal: async (modal: APIModalInteractionResponseCallbackData) => {
            ...
            switch (state) {
                ...
                case ResponseState.Deferred:
                    setState(ResponseState.Replied);
                    await client.interactions.response.create(interaction.id, interaction.token, createModalInteractionResponse(modal));
                    return;
    ...
    return {
        reply: async (message) => {
            ...
            switch (state) {
                ...
                case ResponseState.Deferred:
                    setState(ResponseState.Replied);
                    await client.interactions.response.create(interaction.id, interaction.token, createInteractionResponse_ChannelMessage(message));
                    return;
                case ResponseState.Replied:
                    setState(ResponseState.Replied);
...
const autocompleteStrategy = ({
    ...
    return {
        ...
        edit: async () => { logger.warn("Cannot edit an autocomplete interaction") },
        interaction: interaction
    }
}

export type ApplicationInteractionClient = MessageClient<APIChatInputApplicationCommandInteraction, APIInteractionResponseCallbackData> & ModalClient;
export type ContextMenuInteractionClient = MessageClient<APIContextMenuInteraction, APIInteractionResponseCallbackData> & ModalClient;
export type AutocompleteInteractionClient = MessageClient<APIApplicationCommandAutocompleteInteraction, APICommandAutocompleteInteractionResponseCallbackData, undefined>;
export type ComponentInteractionClient = MessageClient<APIMessageComponentInteraction, APIInteractionResponseCallbackData> & ModalClient;
...
const selectDeferCreator = (interaction: APIUnknownInteraction) => {
    ...
    if (
        isComponentInteraction(interaction) ||
        isModalInteraction(interaction)
    ) {
...
export const dispatchInteractionResponseClient = (resolve: GatewayResultResolver, interaction: APIUnknownInteraction, environment: DiscordEnvironment, rabs: Rabscuttle) => {
    ...
    const intervalHandle = setTimeout(() => {
        ...
        logger.info(`An interaction for '${interactionType} ${interactionName}' took more than ${GatewayTimeout}ms, deferring response`);
    ...
    if (isContextMenuInteraction(interaction)) {
        ...
        if(!command) {
            logger.warn(`A context menu interaction '${interaction.id}' (command name: '${interaction.data.name}') has no plugin`);
    ...
    if (isAutocompleteInteraction(interaction)) {
        ...
        if(!command) {
            logger.warn(`A autocomplete interaction '${interaction.id}' (from command name: '${interaction.data.name}') has no plugin`);
            message.reply({ });
            return;
        }
        tryInvoke(() => {
    ...
    if(isComponentInteraction(interaction)) {
        ...
        if(plugins.length <= 0) {
            logger.warn(`No plugin has a component registered for interaction '${interaction.id}' (component name: '${componentId}')`);
            // returning here the message should just do a non-edit and end the interaction
            message.reply(interaction.message);
            return;
        }
        tryInvoke(() => {
            logger.debug(`Invoking component interaction '${interaction.id}' (component name: ${componentId}) against plugin${plugins.length > 1 ? 's' : ''}: [${plugins.map(x => x.name).join(', ')}]`);
    ...
    if(isModalInteraction(interaction)) {
        ...
        tryInvoke(() => {
            logger.debug(`Invoking modal interaction '${interaction.id}' (modal name: ${modalId}) against plugin${plugins.length > 1 ? 's' : ''}: [${plugins.map(x => x.name).join(', ')}]`);
            plugins.forEach(x => x.onModalSubmit(message, environment));
        })
        return;
    }

    logger.warn(`An interaction '${interaction.id}' (${interactionType} / ${interactionName}) fell through all types and handlers. This should not have happened.`);
    resolve({ status: 400 });
}