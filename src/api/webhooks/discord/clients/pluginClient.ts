...
const recordCaches = (interaction: UnknownInteraction, client: DiscordAPIClient) => {
    ...
    const insertInto = <T>(entries: Record<any, T> | undefined, set: (entry: T) => unknown) => {
        ...
        Object.values(entries).forEach(x => set(x));
    };
...
export const dispatchInteractionResponseClient = <I extends Interaction>(
    ...
    if (isContextMenuInteraction(interaction)) {
        ...
        tryInvoke(() => {
            logger.debug(
                `Invoking context menu interaction '${interaction.id}' against plugin with name '${command.name}'`,
            );
    ...
    if (isAutocompleteInteraction(interaction)) {
        ...
        tryInvoke(() => {
            logger.debug(
                `Invoking autocomplete interaction '${interaction.id}' against plugin with name '${command.name}'`,
            );
    ...
    if (isComponentInteraction(interaction)) {
        ...
        tryInvoke(() => {
            ...
            plugins.forEach(x => x.onNewButtonClick(client, environment));
        });
        return;
    }
    ...
    if (isModalInteraction(interaction)) {
        ...
        if (plugins.length <= 0) {
            logger.warn(
                `No plugin has a modal registered for interaction '${interaction.id}' (modal name: '${modalId}')`,
            );
            // returning here the message should just do a non-edit and end the interaction
        ...
        tryInvoke(() => {
            ...
            plugins.forEach(x => x.onModalSubmit(client, environment));
        });
        return;
    }
